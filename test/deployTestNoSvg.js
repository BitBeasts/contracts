...
describe("Opening Portals", async function () {
  ...
  it("Can only set name on claimed BitBeast", async function () {
    await truffleAssert.reverts(
      aavegotchiGameFacet.setAavegotchiName("1", "Portal"),
      "AavegotchiGameFacet: Must claim BitBeast before setting name"
    );
  });

  it("Should claim an BitBeast", async function () {
    const myPortals = await global.aavegotchiFacet.allAavegotchisOfOwner(
      account
    );
    ...
    const bitbeast = await global.aavegotchiFacet.getAavegotchi(tokenId);

    const collateral = bitbeast.collateral;
    expect(selectedGhost.collateralType).to.equal(collateral);
    expect(bitbeast.status).to.equal(3);
    expect(bitbeast.hauntId).to.equal(0);
    expect(bitbeast.stakedAmount).to.equal(minStake);
