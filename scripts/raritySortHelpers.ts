...
export function _aavegotchiNumericTraits(bitbeast: LeaderboardAavegotchi) {
  const val = bitbeast.withSetsNumericTraits?.reduce(
    (total: number, val: number) => {
      return total + val;
    }
  );

  if (val === 0) return bitbeast.modifiedNumericTraits;
  if (bitbeast.withSetsNumericTraits) return bitbeast.withSetsNumericTraits;
  else return bitbeast.modifiedNumericTraits;
}
