...
const { aavegotchiSvgs } = require('../svgs/bitbeast.js')
...
async function main (scriptName) {
  ...
  let [
    ...
    'contracts/BitBeast/facets/BridgeFacet.sol:BridgeFacet',
  ...
  const aavegotchiDiamond = await diamond.deploy({
    ...
    initDiamond: 'contracts/BitBeast/InitDiamond.sol:InitDiamond',
    facets: [
  ...
  console.log('BitBeast diamond address:' + aavegotchiDiamond.address)

  tx = aavegotchiDiamond.deployTransaction
  receipt = await tx.wait()
  console.log('BitBeast diamond deploy gas used:' + strDisplay(receipt.gasUsed))
  totalGasUsed = totalGasUsed.add(receipt.gasUsed)
  ...
  aavegotchiFacet = await ethers.getContractAt('contracts/BitBeast/facets/AavegotchiFacet.sol:AavegotchiFacet', aavegotchiDiamond.address)
  ...
  bridgeFacet = await ethers.getContractAt('contracts/BitBeast/facets/BridgeFacet.sol:BridgeFacet', aavegotchiDiamond.address)
  ...
  itemsFacet = await ethers.getContractAt('contracts/BitBeast/facets/ItemsFacet.sol:ItemsFacet', aavegotchiDiamond.address)
  ...
  // --------------------------------
  console.log('Uploading bitbeast SVGs')
  ;[svg, svgTypesAndSizes] = setupSvg(
    ['bitbeast', aavegotchiSvgs]
  )
  printSizeInfo(svgTypesAndSizes)
  tx = await svgFacet.storeSvg(svg, svgTypesAndSizes)
  receipt = await tx.wait()
