...
contract SvgViewsFacet is Modifiers {
    ///@notice Get the sideview svgs of an bitbeast
    ///@dev Only valid for claimed aavegotchis
    ///@param _tokenId The identifier of the bitbeast to query
    ...
    function getAavegotchiSideSvgs(uint256 _tokenId) public view returns (string[] memory ag_) {
        ...
        ag_ = new string[](4);
        require(s.aavegotchis[_tokenId].status == LibAavegotchi.STATUS_AAVEGOTCHI, "SvgFacet: BitBeast not claimed");
    ...
    function getAavegotchiSideSvgLayers(
        ...
        // aavagotchi body
        svg_ = LibSvg.getSvg(LibSvg.bytesToBytes32("bitbeast-", _sideView), LibSvg.AAVEGOTCHI_BODY_SVG_ID);
        ...
        //If tokenId is MAX_INT, we're rendering a Portal BitBeast, so no wearables.
        if (_tokenId == type(uint256).max) {
            if (side == back) {
                svg_ = abi.encodePacked(
                    applySideStyles(details, _tokenId, equippedWearables),
                    LibSvg.getSvg(LibSvg.bytesToBytes32("bitbeast-", _sideView), LibSvg.BACKGROUND_SVG_ID),
                    svg_
                );
            } else {
                svg_ = abi.encodePacked(
    ...
    ///@notice Allow the sideview preview of an bitbeast given the haunt id,a set of traits,wearables and collateral type
    ///@param _hauntId Haunt id to use in preview
    ///@param _collateralType The type of collateral to use
    ///@param _numericTraits The numeric traits to use for the bitbeast
    ///@param equippedWearables The set of wearables to wear for the bitbeast
    ...
    function addBodyAndWearableSideSvgLayers(
        ...
        layers.hands = abi.encodePacked(svg_, LibSvg.getSvg(LibSvg.bytesToBytes32("bitbeast-", _sideView), LibSvg.HANDS_SVG_ID));
        ...
        for (uint256 i = 0; i < equippedWearables.length; i++) {
            ...
            if (i == LibItems.WEARABLE_SLOT_BG && wearableId != 0) {
                ...
                layers.background = LibSvg.getSvg("bitbeast", 4);
            }
