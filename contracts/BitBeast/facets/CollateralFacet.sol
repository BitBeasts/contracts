...
///@notice Allow the owner of a claimed bitbeast to increase its collateral stake
///@dev Only valid for claimed aavegotchis
...
///@notice Allow the owner of a claimed bitbeast to decrease its collateral stake
///@dev Only valid for claimed aavegotchis
...
///@notice Allow the owner of an bitbeast to destroy his bitbeast and transfer the XP points to another bitbeast
///@dev Only valid for claimed aavegotchisi
///@dev Name assigned to destroyed bitbeast is freed up for use by another aavegotch
///@param _tokenId Identifier of NFT to destroy
///@param _toId Identifier of another claimed bitbeast where the XP of the sacrificed bitbeast will be sent

    function decreaseAndDestroy(uint256 _tokenId, uint256 _toId) external onlyUnlocked(_tokenId) onlyAavegotchiOwner(_tokenId) {
        address escrow = s.aavegotchis[_tokenId].escrow;
        require(escrow != address(0), "CollateralFacet: Does not have an escrow");

        // require(s.nftItems[address(this)][_tokenId].length == 0, "CollateralFacet: Can't burn bitbeast with items");
