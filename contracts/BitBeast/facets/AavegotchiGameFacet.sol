...
import {Modifiers, Haunt, BitBeast} from "../libraries/LibAppStorage.sol";
...
contract AavegotchiGameFacet is Modifiers {
    ...
    ///@notice Allows the owner of an NFT(Portal) to claim an BitBeast provided it has been unlocked
    ...
    ///@param _tokenId The identifier of NFT to claim an BitBeast from
    ///@param _option The index of the bitbeast to claim(1-10)
    ///@param _stakeAmount Minimum amount of collateral tokens needed to be sent to the new bitbeast escrow contract
    function claimAavegotchi(uint256 _tokenId, uint256 _option, uint256 _stakeAmount) external onlyUnlocked(_tokenId) onlyAavegotchiOwner(_tokenId) {
        BitBeast storage bitbeast = s.aavegotchis[_tokenId];
        require(bitbeast.status == LibAavegotchi.STATUS_OPEN_PORTAL, "AavegotchiGameFacet: Portal not open");
        require(_option < PORTAL_AAVEGOTCHIS_NUM, "AavegotchiGameFacet: Only 10 bitbeast options available");
        ...
        bitbeast.randomNumber = option.randomNumber;
        bitbeast.numericTraits = option.numericTraits;
        bitbeast.collateralType = option.collateralType;
        bitbeast.minimumStake = option.minimumStake;
        bitbeast.lastInteracted = uint40(block.timestamp - 12 hours);
        bitbeast.interactionCount = 50;
        bitbeast.claimTime = uint40(block.timestamp);

        require(_stakeAmount >= option.minimumStake, "AavegotchiGameFacet: _stakeAmount less than minimum stake");

        bitbeast.status = LibAavegotchi.STATUS_AAVEGOTCHI;
        emit ClaimAavegotchi(_tokenId);

        address escrow = address(new CollateralEscrow(option.collateralType));
        bitbeast.escrow = escrow;
    ...
    ///@dev only valid for claimed aavegotchis
    ///@dev Will throw if the name has been used for another claimed bitbeast
    ///@param _tokenId the identifier if the NFT to name
    ///@param _name Preferred name to give the claimed bitbeast
    ...
    function setAavegotchiName(uint256 _tokenId, string calldata _name) external onlyUnlocked(_tokenId) onlyAavegotchiOwner(_tokenId) {
        require(s.aavegotchis[_tokenId].status == LibAavegotchi.STATUS_AAVEGOTCHI, "AavegotchiGameFacet: Must claim BitBeast before setting name");
        ...
        require(!s.aavegotchiNamesUsed[lowerName], "AavegotchiGameFacet: BitBeast name used already");
    ...
    function interact(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i; i < _tokenIds.length; i++) {
            ...
            if (owner != address(this) && owner != 0xD5543237C656f25EEA69f1E247b8Fa59ba353306) {
                // Check lending status of bitbeast and allow original pet operators
            ...
            require(s.aavegotchis[tokenId].status == LibAavegotchi.STATUS_AAVEGOTCHI, "LibAavegotchi: Only valid for BitBeast");
