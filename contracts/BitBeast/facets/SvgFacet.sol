...
contract SvgFacet is Modifiers {
   ...
   |__________________________________*/
    ...
    ///@notice Given an bitbeast token id, return the combined SVG of its layers and its wearables
    ...
    function getAavegotchiSvgLayers(
        ...
        // bitbeast body
        svg_ = LibSvg.getSvg("bitbeast", LibSvg.AAVEGOTCHI_BODY_SVG_ID);
        ...
        if (_tokenId == type(uint256).max) {
            svg_ = abi.encodePacked(
                ...
                LibSvg.getSvg("bitbeast", LibSvg.BACKGROUND_SVG_ID),
                svg_,
                details.collateral,
                details.eyeShape
            );
    ...
    ///@notice Allow the preview of an bitbeast given the haunt id,a set of traits,wearables and collateral type
        ...
        returns (bytes memory svg_)
    {
        ...
        // get hands
        layers.hands = abi.encodePacked(svg_, LibSvg.getSvg("bitbeast", LibSvg.HANDS_SVG_ID));
