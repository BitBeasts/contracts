...
import {LibAppStorage, Modifiers, ItemType, BitBeast, ItemType, WearableSet, NUMERIC_TRAITS_NUM, EQUIPPED_WEARABLE_SLOTS, PORTAL_AAVEGOTCHIS_NUM} from "../libraries/LibAppStorage.sol";
...
contract ItemsFacet is Modifiers {
   ...
   |__________________________________*/
    ...
    ///@notice Allow the owner of a claimed bitbeast to equip/unequip wearables to his bitbeast
    ///@dev Only valid for claimed aavegotchis
    ...
    ///@param _tokenId The identifier of the bitbeast to make changes to
        ...
        onlyUnlocked(_tokenId)
    {
        ...
        require(bitbeast.status == LibAavegotchi.STATUS_AAVEGOTCHI, "LibAavegotchi: Only valid for AG");
        emit EquipWearables(_tokenId, bitbeast.equippedWearables, _wearablesToEquip);

        address sender = LibMeta.msgSender();

        for (uint256 slot; slot < EQUIPPED_WEARABLE_SLOTS; slot++) {
            uint256 toEquipId = _wearablesToEquip[slot];
            uint256 existingEquippedWearableId = bitbeast.equippedWearables[slot];
            ...
            bitbeast.equippedWearables[slot] = uint16(toEquipId);
            ...
            if (existingEquippedWearableId != 0 && s.itemTypes[existingEquippedWearableId].canBeTransferred) {
                // remove wearable from BitBeast and transfer item to owner
            ...
            if (toEquipId != 0) {
                ...
                require(LibAavegotchi.aavegotchiLevel(bitbeast.experience) >= itemType.minLevel, "ItemsFacet: AG level lower than minLevel");
                ...
                require(itemType.slotPositions[slot] == true, "ItemsFacet: Wearable can't be equipped in slot");
                {
                    ...
                    if (allowedCollaterals.length > 0) {
                        uint256 collateralIndex = s.collateralTypeIndexes[bitbeast.collateralType];
    ...
    ///@notice Allow the owner of an NFT to use multiple consumable items for his bitbeast
    ///@dev Only valid for claimed aavegotchis
    ///@dev Consumables can be used to boost kinship/XP of an bitbeast
    ///@param _tokenId Identtifier of bitbeast to use the consumables on
