...
contract ForgeFacet is Modifiers {
    ...
    modifier onlyAavegotchiUnlocked(uint256 gotchiId) {
        require(!aavegotchiGameFacet().isAavegotchiLocked(gotchiId), "ForgeFacet: BitBeast is locked");
        _;
    }
    // @notice Will revert if bitbeast is in active rental as well.
    modifier onlyAavegotchiOwner(uint256 gotchiId) {
        require(!lendingGetterAndSetterFacet().isAavegotchiLent(uint32(gotchiId)), "ForgeFacet: BitBeast is lent out");
        require(LibMeta.msgSender() == aavegotchiFacet().ownerOf(gotchiId), "ForgeFacet: Not BitBeast owner");
        _;
    }
    ...
    // @notice Get an BitBeast's current smithing skill level
    ...
    // @notice Get the specific Geode token ID given an BitBeast rarity score modifier.
    ...
    function _forge(
        ...
        require(!s.gotchiForging[gotchiId].isForging, "ForgeFacet: BitBeast already forging");
    ...
    // @dev returns the time (in blocks) cost for forging for an bitbeast
    // @param gotchId
    ...
    function _getForgeQueueItem(uint256 gotchiId) internal view returns (ForgeQueueItem storage output) {
        require(s.gotchiForging[gotchiId].isForging, "ForgeFacet: BitBeast not currently forging.");
    ...
    // @notice Allow BitBeast diamond to mint essence.
    // @dev Only called from CollateralFacet's decreaseAndDestroy function. Not including a whenNotPaused modifier
    //      here to avoid impacts to bitbeast sacrifice functionality.
    function mintEssence(
        address owner /*uint256 gotchiId*/
    ) external {
        require(LibMeta.msgSender() == ForgeLibDiamond.AAVEGOTCHI_DIAMOND, "ForgeFacet: Can only be called by BitBeast Diamond");
        //        require(aavegotchiFacet.ownerOf(gotchiId) == address(0), "ForgeFacet: BitBeast not sacrificed");
