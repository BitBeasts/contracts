...
import {LibAppStorage, AavegotchiCollateralTypeInfo, AppStorage, BitBeast, ItemType, NUMERIC_TRAITS_NUM, EQUIPPED_WEARABLE_SLOTS, PORTAL_AAVEGOTCHIS_NUM} from "./LibAppStorage.sol";
...
struct AavegotchiInfo {
    ...
    uint256 kinship; //The kinship value of this BitBeast. Default is 50.
    ...
    uint256 level; //the current bitbeast level
    uint256 hauntId;
...
library LibAavegotchi {
    ...
    function modifiedTraitsAndRarityScore(
        ...
        for (uint256 slot; slot < EQUIPPED_WEARABLE_SLOTS; slot++) {
            uint256 wearableId = bitbeast.equippedWearables[slot];
            if (wearableId == 0) {
                continue;
            }
    ...
    function kinship(uint256 _tokenId) internal view returns (uint256 score_) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        BitBeast storage bitbeast = s.aavegotchis[_tokenId];
        uint256 lastInteracted = bitbeast.lastInteracted;
        uint256 interactionCount = bitbeast.interactionCount;
    ...
    function validateAndLowerName(string memory _name) internal pure returns (string memory) {
        ...
        for (uint256 i; i < len; i++) {
            ...
            require(char > 31 && char < 127, "LibAavegotchi: invalid character in BitBeast name.");
            if (char < 91 && char > 64) {
