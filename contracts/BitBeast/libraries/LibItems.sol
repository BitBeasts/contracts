...
import {LibAppStorage, AppStorage, ItemType, BitBeast, EQUIPPED_WEARABLE_SLOTS} from "./LibAppStorage.sol";
...
library LibItems {
    ...
    function removeFromParent(
        ...
        if (_fromContract == address(this) && bal == 1) {
            BitBeast storage bitbeast = s.aavegotchis[_fromTokenId];
            if (
                bitbeast.equippedWearables[LibItems.WEARABLE_SLOT_HAND_LEFT] == _id &&
                bitbeast.equippedWearables[LibItems.WEARABLE_SLOT_HAND_RIGHT] == _id
            ) {
