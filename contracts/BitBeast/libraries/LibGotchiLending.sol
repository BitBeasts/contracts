...
library LibGotchiLending {
    ...
    function verifyAddGotchiLendingParams(
        ...
        require(s.aavegotchis[_erc721TokenId].status == LibAavegotchi.STATUS_AAVEGOTCHI, "LibGotchiLending: Can only lend BitBeast");
    ...
    function enforceAavegotchiNotInLending(uint32 _tokenId, address _sender) internal {
        ...
        if (_listingId > 0) {
            ...
            require(_lending.lender == _sender, "LibGotchiLending: BitBeast is in lending");
            if (_lending.timeAgreed > 0) {
                // revert if agreed lending
                revert("LibGotchiLending: BitBeast is in lending");
            } else {
