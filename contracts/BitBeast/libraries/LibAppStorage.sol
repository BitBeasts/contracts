...
struct BitBeast {
    uint16[EQUIPPED_WEARABLE_SLOTS] equippedWearables; //The currently equipped wearables of the BitBeast
    ...
    uint256 experience; //How much XP this BitBeast has accrued. Begins at 0.
    uint256 minimumStake; //The minimum amount of collateral that must be staked. Set upon creation.
    uint256 usedSkillPoints; //The number of skill points this bitbeast has already used
    uint256 interactionCount; //How many times the owner of this BitBeast has interacted with it.
    address collateralType;
    uint40 claimTime; //The block timestamp when this BitBeast was claimed
    uint40 lastTemporaryBoost;
    uint16 hauntId;
    address owner;
    uint8 status; // 0 == portal, 1 == VRF_PENDING, 2 == open portal, 3 == BitBeast
    uint40 lastInteracted; //The last time this BitBeast was interacted with
    bool locked;
    address escrow; //The escrow address this BitBeast manages.
}
...
struct ItemType {
    ...
    uint16 minLevel; //The minimum BitBeast level required to use this item. Default is 1.
...
struct ERC721Listing {
    ...
    uint256 category; // 0 is closed portal, 1 is vrf pending, 2 is open portal, 3 is BitBeast
    uint256 priceInWei;
    uint256 timeCreated;
...
struct AppStorage {
    ...
    mapping(uint256 => BitBeast) aavegotchis;
...
contract Modifiers {
    ...
    modifier onlyAavegotchiOwner(uint256 _tokenId) {
        require(LibMeta.msgSender() == s.aavegotchis[_tokenId].owner, "LibAppStorage: Only bitbeast owner can call this function");
        _;
    }
